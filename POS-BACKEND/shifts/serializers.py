...
class ShiftSerializer(serializers.ModelSerializer):
    ...
    def get_transaction_count(self, obj):
        try:
            return self._get_non_void_sales(obj).count()
        except:
            return 0

    def _get_non_void_sales(self, obj):
        sales = getattr(obj, '_prefetched_objects_cache', {}).get('sale_set')
        if sales is not None:
            return [sale for sale in sales if not getattr(sale, 'voided', False)]
        return obj.sale_set.filter(voided=False)

    def get_total_sales(self, obj):
        sales = self._get_non_void_sales(obj)
        if isinstance(sales, list):
            return float(sum(float(getattr(sale, 'final_amount', 0) or 0) for sale in sales))
        result = sales.aggregate(total=Sum('final_amount'))
    ...
    def get_net_sales(self, obj):
        return self.get_total_sales(obj) - self.get_total_returns(obj)
