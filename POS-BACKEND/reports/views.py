...
class ReportViewSet(viewsets.ModelViewSet):
    ...
    def _get_daily_sales_summary(self, date=None):
        ...
        for item in sales_items:
            if item.product and item.product.cost_price:
                ...
                cost_of_goods_sold += float(item.product.cost_price * item.quantity)
    ...
    def _calculate_profit_loss(self, date_from, date_to):
        ...
        # Estimated operating expenses (5% of gross profit)
        operating_expenses = gross_profit * 0.05
...
class SalesSummaryView(generics.GenericAPIView):
    ...
    def _get_shift_sales_data(self, shift_id):
        ...
        # Get voided held orders for the cashier during this shift period
        voided_held_orders = Cart.objects.filter(
            cashier=cashier,
            status='voided',
            updated_at__gte=shift.start_time,
            updated_at__lte=shift.end_time or timezone.now()
        ).select_related('customer').prefetch_related('cartitem_set__product').order_by('-updated_at')
            ...
            'voided_held_orders': [
                {
                    ...
                    'voided_at': cart.updated_at.isoformat() if cart.updated_at else None,
                    'items': [
                        {
                            'product_name': item.product.name,
                            'quantity': item.quantity,
                ...
                for cart in voided_held_orders
            ],
            'held_orders': [
                {
                    'id': cart.id,
    ...
    def _get_all_shifts_data(self, date_from=None, date_to=None, user_filter=None):
        ...
        if user_filter:
            shifts_query = shifts_query.filter(cashier__user__username__icontains=user_filter)
        ...
        for shift in shifts:
            ...
            # Get voided held orders for this shift
            voided_held_orders = Cart.objects.filter(
                cashier=shift.cashier,
                status='voided',
                updated_at__gte=shift.start_time,
            ...
            if (
                not shift_sales.exists()
                and not voided_sales.exists()
                and not voided_held_orders.exists()
                and not held_orders.exists()
                and not shift_returns.exists()
            ):
                continue
            ...
            all_voided_sales.extend(shift_summaries[-1]['voided_sales'])
            all_voided_held_orders.extend(shift_summaries[-1]['voided_held_orders'])
        ...
        result = {
            ...
            'voided_sales': all_voided_sales,
            'voided_held_orders': all_voided_held_orders,
        }
    ...
    def get(self, request, sale_id=None):
        ...
        if all_shifts or all_users:
            # Return all shifts summary data
            date_from = request.query_params.get('date_from')
            date_to = request.query_params.get('date_to')
            sales_data = self._get_all_shifts_data(date_from, date_to, user_filter=user_filter)
    ...
    def _get_today_summary(self):
        ...
        # Today's actual profit calculation
        from sales.models import SaleItem
        sales_items_today = SaleItem.objects.filter(
            sale__sale_date__date=today,
            sale__voided=False
        ).select_related('product')

        today_profit = 0
        for item in sales_items_today:
            if item.product and item.product.cost_price:
                profit_per_item = (item.unit_price - item.product.cost_price) * item.quantity
                today_profit += float(profit_per_item)

        # Apply 5% operating cost deduction for net profit
        today_profit = today_profit * 0.95
